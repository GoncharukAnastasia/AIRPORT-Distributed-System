...
function draw() {
    ...
    if (snapshotData && snapshotData.gc) {
        ...
        if (gc.nodes) gc.nodes.forEach(n => { nodeOcc[n.name] = n.occupiedBy || []; });
        if (gc.edges) gc.edges.forEach(e => { edgeOcc[e.name] = e.occupiedBy || []; });
    }
...
(async function init() {
    ...
    resize();
    await fetchSnapshot();
    setInterval(fetchSnapshot, 2000); // poll every 2 seconds
})();
</script>
</body>
</html>
