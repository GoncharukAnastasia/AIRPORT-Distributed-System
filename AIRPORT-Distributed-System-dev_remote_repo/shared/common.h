...
inline HttpResult http_put_json(const std::string& host, int port, const std::string& path, const json& payload) {
    HttpResult out;
    try {
        httplib::Client cli(host, port);
        cli.set_connection_timeout(2, 0);
...
inline HttpResult http_patch_json(const std::string& host, int port, const std::string& path, const json& payload) {
    HttpResult out;
    try {
        httplib::Client cli(host, port);
        cli.set_connection_timeout(2, 0);
        ...
        auto res = cli.Patch(path.c_str(), payload.dump(), "application/json");
        if (!res) {
            out.error = "connection failed";
            return out;
        }
