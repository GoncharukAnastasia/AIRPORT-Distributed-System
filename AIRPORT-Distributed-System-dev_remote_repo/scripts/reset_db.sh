...
-- Таблица ожидается: info_flights
-- Колонки: flight_id, scheduled_at, scheduled_time, status, phase, parking_node, updated_at
-- =========================

-- Если хочешь строго оставить только 2 рейса, раскомментируй:
-- DELETE FROM info_flights
-- WHERE flight_id NOT IN ('$FLIGHT1_ID', '$FLIGHT2_ID');

INSERT INTO info_flights (flight_id, scheduled_at, scheduled_time, status, phase, parking_node, updated_at)
VALUES
  ('$FLIGHT1_ID', $NOW_EPOCH, to_char(now(), 'YYYY-MM-DD"T"HH24:MI:SS"Z"'), 'Scheduled', 'airborne', NULL, $NOW_EPOCH),
  ('$FLIGHT2_ID', $NOW_EPOCH, to_char(now(), 'YYYY-MM-DD"T"HH24:MI:SS"Z"'), 'Parked', 'grounded', '$FLIGHT2_PARKING', $NOW_EPOCH)
