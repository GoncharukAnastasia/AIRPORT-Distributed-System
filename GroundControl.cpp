...
public:
    ...
    {
        httplib::Server svr;
        app::enable_http_access_log(svr, "GroundControl");
        ...
        // Manual grounded spawn: place plane directly onto parking node.
        // POST /v1/flights/{flightId}/spawn_grounded  body: {"parkingNode":"P-4"}
        svr.Post(R"(/v1/flights/([A-Za-z0-9\-_]+)/spawn_grounded)", [&](const httplib::Request& req, httplib::Response& res)
        {
            const std::string flightId = req.matches[1];
            if (flightId.empty()) {
            ...
            auto bodyOpt = app::parse_json_body(req);
            if (bodyOpt) {
                parkingNode = app::s_or(*bodyOpt, "parkingNode");
            }
            ...
            if (parkingNode.empty()) {
                app::reply_json(res, 400, {{"error", "parkingNode required"}});
                return;
            }

            {
                std::lock_guard<std::mutex> lk(mtx_);
                if (!nodes_.count(parkingNode)) {
                    app::reply_json(res, 404, {{"error", "parking node not found"}, {"parkingNode", parkingNode}});
                    return;
                }

                if (!node_has_plane_slot_unsafe_(parkingNode) &&
                    nodeOcc_[parkingNode].count("plane:" + flightId) == 0) {
                    app::reply_json(res, 409, {{"error", "parking already has a plane"}, {"parkingNode", parkingNode}});
                    return;
                }

                place_plane_unsafe_(flightId, parkingNode);
                ...
                push_event_unsafe("flight.spawned_grounded", {
                    {"flightId", flightId},
                    {"parking", parkingNode}
                });
            }

            notify_panel_status(flightId, "Parked", "spawn_grounded", parkingNode);
            app::reply_json(res, 200, {{"ok", true}, {"flightId", flightId}, {"parkingNode", parkingNode}});
        });
        ...
        svr.Post(R"(/v1/flights/([A-Za-z0-9\-_]+)/landed)", [&](const httplib::Request& req, httplib::Response& res)
        {
            ...
            // Idempotency: if this flight is already at RE-1, return success.
            auto currentNode = find_plane_node_unsafe(flightId);
            if (currentNode && *currentNode == kLandingNode)
            {
            ...
            // Replace RE-1 reservation with the plane position, moving plane from any old node.
        ...
        // ── Maintenance: forcefully evict a plane and reset its session ──────
        // POST /v1/maintenance/clear   body: {"flightId":"SU100"}
        // Removes plane:* from all nodes, releases parking/RW reservations,
        // and deletes the session.  Safe to call at any time for demo resets.
        svr.Post("/v1/maintenance/clear", [&](const httplib::Request& req, httplib::Response& res)
        {
            auto bodyOpt = app::parse_json_body(req);
            if (!bodyOpt) {
                app::reply_json(res, 400, {{"error", "invalid json"}});
            ...
            // 1) Remove plane token from every node
            const std::string planeToken  = "plane:" + flightId;
            const std::string resToken    = "reservation:" + flightId;
            const std::string tkResToken  = "takeoff_reservation:" + flightId;
            for (auto& [node, occ] : nodeOcc_) {
                occ.erase(planeToken);
                occ.erase(resToken);
                occ.erase(tkResToken);
            }

            // 2) Release reserved parking (if any)
            auto it = sessions_.find(flightId);
            if (it != sessions_.end()) {
            ...
            // 3) Release corridor if this flight held it
            corridor_release_if_holder_unsafe_(flightId, "maintenance_clear");

            push_event_unsafe("maintenance.cleared", {{"flightId", flightId}});
